...
from ccb.cli import parse_args, collect_sources, convert_single, ComicBookConverter, process_paths
import importlib
...
class TestCLI:
    ...
    def test_parse_args(self, monkeypatch):
        monkeypatch.setattr('sys.argv', ['ccb', 'input_path', '-f', 'folder', '-t', 'cbz', '-o', 'outdir', '-c', '-q', '-R', '-F'])
        args = parse_args()
    ...
    def test_convert_single_with_dummy_converter(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            input_path.mkdir()
            result = asyncio.run(convert_single(converter, input_path, None, 'cbz', None, False, False))
